...
def image_callback(image_left, image_right, info_left, info_right):
    ...
    R = np.array([[ 0.99997795, -0.00600599, -0.00283236],
                [ 0.00598826,  0.99996269, -0.00622629],
                [ 0.00286965,  0.0062092,   0.99997658]], np.float32)
    T = np.array([-6.34615570e-02, -2.11421808e-04,  4.06257022e-05], np.float32)
    ...
    # print("stereo focal length undistorted:",stereo_focal_px)
    ...
    cv2.imshow(WINDOW_TITLE, disp_color)
    cv2.imshow("color_image", color_image)
    cv2.waitKey(1)

    xyz = cv2.reprojectImageTo3D(disparity,Q,False)
